...
check(type(p2.cog.capability) == "function", "capability_function")
...
check(required.find("cleanup_all_policy") != nil, "required_cleanup_all_policy")
check(required.find("isolated_vm_closure") != nil, "required_isolated_vm")
check(required.find("isolated_child_vm_cog_policy") != nil, "required_isolated_policy")
check(required.find("isolated_child_vm_cog_reason") != nil, "required_isolated_reason")
...
check(p2.cog.capability("native_blink") == caps["native_blink"], "capability_native_blink")
check(caps["native_blink_stack"] == 2048, "native_blink_stack")
check(caps["handle_model"] == "native_blink_info_stop_only", "handle_model")
check(p2.cog.capability("handle_model") == caps["handle_model"], "capability_handle_model")
...
check(caps["cleanup_all"], "cleanup_all")
check(caps["cleanup_all_policy"] == "stop_releases_all_spawned_closure_cog_slots", "cleanup_all_policy")
check(p2.cog.capability("cleanup_all_policy") == caps["cleanup_all_policy"], "capability_cleanup_all_policy")
...
check(caps["isolated_child_vm_cog_policy"] == "unsupported_catalina_xmm_c_cog_runtime_not_safe", "isolated_child_policy")
check(p2.cog.capability("isolated_child_vm_cog_policy") == caps["isolated_child_vm_cog_policy"], "capability_isolated_policy")
check(string.find(caps["isolated_child_vm_cog_reason"], "Catalina XMM") != nil, "isolated_child_reason")
check(p2.cog.capability("isolated_child_vm_cog_reason") == caps["isolated_child_vm_cog_reason"], "capability_isolated_reason")
check(!caps["isolated_vm_closure"], "isolated_vm_closure_false")
check(!caps["literal_closure_transfer"], "literal_transfer_false")
check(p2.cog.capability("missing") == nil, "capability_missing")
check(p2.cog.capability(nil) == nil, "capability_nil")
...
check(audit["cleanup_all_policy"] == caps["cleanup_all_policy"], "audit_cleanup_all_policy")
check(audit["isolated_child_vm_cog_policy"] == caps["isolated_child_vm_cog_policy"], "audit_isolated_policy")
check(audit["isolated_child_vm_cog_reason"] == caps["isolated_child_vm_cog_reason"], "audit_isolated_reason")
