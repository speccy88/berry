...
def has_task_member(name)
    return introspect.contains(task, name)
end
...
result_task = task.start(step, "r", 2)
first_step = task.next_result()
...
run_summary = task.run_result(4, 0)
...
assert(task.next() == w)
assert(task.status(w) == "waiting")
life = task.lifecycle_result(w)
assert(life["ok"])
assert(life["allocated"])
assert(life["status"] == "waiting")
assert(life["waiting"])
assert(life["wait"] == "event")
assert(life["wait_event"] == "go")
assert(life["has_deadline"])
assert(life["remaining_ms"] >= 0)
assert(life["current_vm"])
assert(task.signal("go"))
assert(task.next() == w)
assert(woke["event"])
assert(task.status(w) == "free")
...
assert(life["ok"])
assert(!life["allocated"])
assert(life["status"] == "free")
assert(life["error"] == "free")
invalid_life = task.lifecycle_result(-99)
...
ready_queue_get = task.ready_result(ready_queue)
assert(ready_queue_get["ready"])
ready_queue_put = task.ready_result(ready_queue, "put")
assert(!ready_queue_put["ready"])
assert(ready_queue_put["wait"] == "queue_put")
ready_channel = p2ipc.channel.new(1)
ready_channel_recv = task.ready_result(ready_channel)
...
assert(ready_channel_recv["ready"])
ready_channel_send = task.ready_result(ready_channel, "send")
assert(!ready_channel_send["ready"])
ready_mailbox = p2ipc.mailbox.new()
ready_mailbox_get = task.ready_result(ready_mailbox)
...
ready_timer = task.Timer(0, false)
...
task.reset()
closed_ch = p2ipc.channel.new(1)
closed_recv_state = {"error": nil}
...
closed_recv_h = task.start(ipc_channel_closed_recv_waiter, closed_recv_state, closed_ch)
...
task.reset()
closed_full_ch = p2ipc.channel.new(1)
assert(closed_full_ch.send_result("held")["ok"])
closed_send_state = {"error": nil}
...
closed_send_h = task.start(ipc_channel_closed_send_waiter, closed_send_state, closed_full_ch)
...
task.reset()
closed_mb = p2ipc.mailbox.new()
closed_get_state = {"error": nil}
...
closed_get_h = task.start(ipc_mailbox_closed_get_waiter, closed_get_state, closed_mb)
...
task.reset()
closed_full_mb = p2ipc.mailbox.new()
assert(closed_full_mb.put_result("held")["ok"])
closed_put_state = {"error": nil}
...
closed_put_h = task.start(ipc_mailbox_closed_put_waiter, closed_put_state, closed_full_mb)
...
reset_cleanup_h = task.start(reset_noop)
assert(reset_cleanup_h >= 0)
assert(task.signal("reset_event"))
reset_report = task.reset_result()
