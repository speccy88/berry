...
assert(isinstance(bytecode_execution, map))
...
if plan["manifest_template_available"]
    ...
    assert(isinstance(plan["manifest_template"], map))
...
assert(isinstance(status["load_plan"], map))
assert(isinstance(status["emit_plan"], map))
assert(isinstance(status["validation"], map))
...
assert(isinstance(emit_plan_many["reasons"], map))
