...
assert(math_close(accel["turn"], 4294967296.0, 4096))
...
assert(p2.cogid() >= 0)
assert(p2.cogid() < 8)
assert(isinstance(p2.heap_info(), map))
assert(isinstance(p2.cog_states(), list))
var status = p2.status_info()
assert(isinstance(status, map))
...
assert(isinstance(status["cogs"], list))
