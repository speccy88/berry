...
assert(heap_caps["audit_policy"] == "tiny_in_memory_heap_self_check")
...
assert(heap_audit["audit_policy"] == "tiny_in_memory_heap_self_check")
...
var status_error_bytecode = p2compat.bytecode()
p2.raise_status_info = false
assert(status_error_bytecode["default_source_fallback"])
assert(!status_error_bytecode["cache_emit"])
assert(!status_error_bytecode["execution"])
