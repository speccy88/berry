...
```sh
make spin2 TOOLCHAIN=catalina CATALINA_DIR=.third_party_cache/catalina-v8.8.9-build
```

Temporary RAM SD-loader build:

```sh
make spin2-sd-loader TOOLCHAIN=catalina CATALINA_USE_DOCKER=1 \
  CATALINA_DIR=.third_party_cache/catalina-v8.8.9-build
```
