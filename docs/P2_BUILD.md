...
- `full`: the current no-PSRAM P2 Edge build. Current verified image: `516448` bytes with the existing `128 KiB` main heap, `32 KiB` worker heap, and the `rtos` module enabled. Experimental `worker` and `threads` modules are folded under the public `rtos` API.
