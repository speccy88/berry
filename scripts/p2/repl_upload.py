...
def ensure_dirs(ser, target_path: str, line_ending: bytes, prompt: bytes, timeout: float) -> None:
    ...
    for part in parts:
        ...
        run_command_allow_error(ser, command, line_ending, prompt, timeout)
...
def main() -> int:
    ...
    if not args.recursive_directory and not args.file:
        parser.error("provide --file or --recursive-directory")
