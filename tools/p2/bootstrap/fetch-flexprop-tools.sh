...
# Fetch FlexProp host tools (and matching headers) into a local cache.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
TOOLCHAIN_DIR="${1:-${ROOT_DIR}/.third_party_cache/flexprop}"
...
echo "[1/4] Preparing FlexProp cache..."
mkdir -p "${BIN_DIR}"
mkdir -p "${INC_DIR}"
...
for t in flexcc flexspin loadp2 proploader; do
    if [ -f "${BIN_DIR}/${t}.mac" ] && [ ! -f "${BIN_DIR}/${t}" ]; then
        cp -f "${BIN_DIR}/${t}.mac" "${BIN_DIR}/${t}"
        chmod +x "${BIN_DIR}/${t}" 2>/dev/null || true
    fi
done
