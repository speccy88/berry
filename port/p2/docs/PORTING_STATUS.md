...
- P2 cached module loading is live-verified after the Catalina const native function hang fix:
 ...
 - standalone XMM flash boot passes `/TESTS/P2/XMMGC.BE`
 (`tests/p2/smoke_xmm_gc_stress.be`): retained Berry lists/maps/strings and
 closures survived forced `p2.gc()`, four temporary churn rounds reclaimed back
 to the same free-heap level, the main heap stayed inside the lower
 pointer-addressable PSRAM window, and final free heap ended only `800` bytes
 below the test baseline
