/* P2-only copy of src/be_api.c with be_isge() split out to avoid a Windows FlexC crash near EOF. */
/********************************************************************
...
#if defined(BE_P2_STARTUP_TRACE) && BE_P2_STARTUP_TRACE
static void api_trace(const char *msg)
{
    p2_serial_puts("[api] ");
    p2_serial_puts(msg);
    p2_serial_puts("\n");
